...
**Cсылки на дистрибутив Оркестратора предоставляются по запросу вендору.** Возможны два варианта установки [с помощью докер-контейнера и без](../sherpa-rpa/sherpa-orchestrator/ustanovka-sherpa-orchestrator/). Для проверки контрольной суммы можно использовать Windows утилиту «certutil»: `certutil -hashfile путь_к_файлу` .
