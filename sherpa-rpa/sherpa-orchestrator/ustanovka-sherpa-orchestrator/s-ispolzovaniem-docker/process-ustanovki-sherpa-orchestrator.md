...
Откройте файл конфигурации домена и настройте его:

```bash
# Откройте файл domain.conf в текстовом редакторе
sudo nano ./backend/config/domain.conf
```

**Необходимо заменить домен "orchestrator.sherparpa.ru" на свой во всех местах и сохранить файл (всего 4 изменения).**

<details>

<summary>💡 Комментарии к настройке домена</summary>

**sudo nano ./backend/config/domain.conf** - открывает файл конфигурации домена

* `sudo` - права администратора для редактирования системных файлов
* `nano` - текстовый редактор
* `./backend/config/domain.conf` - путь к файлу конфигурации

**Что нужно изменить:**

* Замените "orchestrator.sherparpa.ru" на ваш домен или IP-адрес
* Всего нужно изменить 4 вхождения домена в файле
